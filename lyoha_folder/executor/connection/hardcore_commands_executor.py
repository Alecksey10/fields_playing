...
from executor.driver.driver import Driver
...
from executor.connection.executor_api_threaded_base import ExecutorAPIThreadedBase
...
class HardcoreCommandsExecutor(ExecutorAPIThreadedBase):
    ...
    async def execute_command(self, command:CommandBase):
    ...
    def _run_loop(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                self.__result_queue.put((True, result))
            except Exception as ex:
                logger.debug("some exeption" + str(ex))
                logger.debug(traceback.format_exc())
                self.__result_queue.put((False, None))
            time.sleep(1)
