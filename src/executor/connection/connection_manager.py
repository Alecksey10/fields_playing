...
sys.path.append('.')
from src.commands.command_fabric import CommandFabric
from src.connection_states.state_connection_closed import StateConnectionClosed
from src.schemas.schemas import DefaultCommand, ExecutorCommandResult, Message
from src.connection_states.state_receive_command import StateReceiveCommand
from src.connection_states.state_send_command_result import StateSendCommandResult
...
class ConnectionManager():
    def __init__(self, executor:ExecutorAPIBase, ws:websockets.client.ClientConnection):
        ...
        self.websocket = ws 
        self.last_command_result:ExecutorCommandResult = None
    ...
    async def __state_receive_command(self, message_obj:Message):
        ...
        self.last_command_result = await self.executor.execute_command(command)
    ...
    async def __state_send_command_result(self):
        ...
        if (self.last_command_result)==None:
            raise Exception("there is no result")
        data=json.dumps({'sucess':self.last_command_result.sucess})
        
        status = 'success' if self.last_command_result.sucess else 'error'
        message_obj = Message(
            status=status,
            data=data
        )
