...
from src.commands.bilet_commands.end_bilet import EndBilet
from src.commands.bilet_commands.start_bilet import StartBilet
...
class CommandsGenerator():
    '''
    Класс для создания последовательности действий при управлении каким-либо executor. 
    Логика направлена на удовлетворение уникальному характеру работы с сайтом pdd через браузер... (там что-то подвисает, вкладки сами переключаются и т.д.), хотя были попытки избавиться от привязки серверной логики к логике клиента executor.
    Удобство - каждый раз пересматривает текущее состояние билета, то есть его можно менять на лету. 
    #TODO, определённо, для различных реализаций executor следовало бы сделать свои генераторы команд, например WebdriverCommandGenerator.
    '''
